...
void logic_simulation_evaluation (map<int, char> testVector, string fault, vector<char>& currentOutputVector, int mode){
   NSTRUC *np;
   int i, j ,k;
   threeValue temp;
   ...
   if (mode == 1){
      ...
      for(itr1 = testVector.begin(); itr1 != testVector.end(); itr1++){
         ...
         for (int b = 0; b < Nnodes; b ++){
            np = &Node[b];
            if(np->num == itr1->first) np->val = testVector[itr1->first];
...
int logicsim(char *cp)
{
   ...
   for (int a = 0; a < allInput[tempInputPorts.at(0)].size(); a ++){
      ...
      vector<char> tempVec;
      logic_simulation_evaluation(newTestVector,"", tempVec,0);
      for(i = 0; i < Npo; i++){
         if (i == 0){
...
int pfs_single(map<int, char> testPatternSingle, string fault, vector<vector<char> >& allOutputVector, int index){
   vector <char> outputVector;
   //cout << "in pfs_single: fault " << fault << endl;
   logic_simulation_evaluation(testPatternSingle, fault, outputVector,1);
   allOutputVector.at(index).push_back(outputVector.at(0));
   allOutputVector.at(index).push_back(outputVector.at(1));
...
int pfs_golden(map<int, char> testPatternSingle, vector<vector<char> >& allOutputVector){
   vector <char> outputVector;
   logic_simulation_evaluation(testPatternSingle, "", outputVector,0);
   allOutputVector.at(0).push_back(outputVector.at(0));
   allOutputVector.at(0).push_back(outputVector.at(1));
   ...
   while (i<testVectorNum){
     ...
     // cout << "thread_width " << thread_width << endl;
      ...
      while(j < faultList.size()){
         ...
         t[counter]= std::thread(pfs_single, newTestVector, faultList[j], std::ref(outputThread), counter);
         counter+= 1;
         totalRun += 1;
         single_totalRun += 1;
         j += 1;
    ...
    //  cout << "-----------------------------" << endl;
      for (int p = 1; p < thread_width; p ++){
         ...
         //cout << outputThread.at(p).at(0) << " " << outputThread.at(p).at(1) << endl;
         if (outputThread.at(p).at(0) != outputThread.at(0).at(0) || outputThread.at(p).at(1) != outputThread.at(0).at(1)){
               int indention;
               if (j > 63){
                  
                  indention = j/63;
                  indention = indention* 63;
               ...
               // }
               if (find(detectedFault.begin(), detectedFault.end(), faultList.at(p-1+indention)) == detectedFault.end()){
                  detectedFault.push_back(faultList.at(p-1+indention));
               }
...
int rtg(char *cp){
   ...
   while (currentTestVecNum < nTot){
      ...
      for (int j = 0; j < allFaultList.size(); j ++){
         ...
         if (std::find(detectedFault.begin(), detectedFault.end(), allFaultList.at(j)) != detectedFault.end()){
            detectedCount += 1;
         }
